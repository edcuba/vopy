...
from vopy.keypoints import harris, select_keypoints, describe_keypoints, match_descriptors, get_fundamental_matrix
from vopy.plotting import plot_frame_shift
...
for i in range(1, LAST_FRAME+1):
    ...
    model, query_inliers, db_inliers = get_fundamental_matrix(matched_query, matched_db, max_trials=RANSAC_TRIALS)

    plot_frame_shift(ax, im, query_inliers, db_inliers)
    print(f"Frame {i} matched {matched_query.shape[0]}/{keypoints.shape[0]} inliers {query_inliers.shape[0]}")

    # TODO: recover camera pose
    # TODO: construct the point cloud
